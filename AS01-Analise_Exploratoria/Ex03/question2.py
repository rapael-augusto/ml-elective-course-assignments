...
#    axes[i].hexbin(df_skin[var_x], df_skin[var_y], cmap='Reds', mincnt=1)
#    axes[i].set_xlabel(f'{var_x}')
#    axes[i].set_ylabel(f'{var_y}')
#    axes[i].set_title(f'skin: {var_x} vs {var_y}')

# for i, (var_x, var_y) in enumerate(pairs):
#    axes[i+3].hexbin(df_non_skin[var_x], df_non_skin[var_y], cmap='Blues', mincnt=1)
...
#    for j, var_y in enumerate(columns[:-1]):
#        if i != j:
#            sns.kdeplot(data=df_sample, x=var_x, y=var_y, ax=axes[p_index], fill=False)
#            axes[p_index].set_xlabel(var_x)
#            axes[p_index].set_ylabel(var_y)
...
# for i, (var_x, var_y) in enumerate(pairs):
#    sns.kdeplot(data=df_skin_sample, x=var_x, y=var_y, ax=axes[i], cmap='Reds')
#    axes[i].set_xlabel(f'{var_x}')
#    axes[i].set_ylabel(f'{var_y}')
#    axes[i].set_title(f'Skin - {var_x} vs {var_y}')

# for i, (var_x, var_y) in enumerate(pairs):
#    sns.kdeplot(data=df_non_skin_sample, x=var_x, y=var_y, ax=axes[i+3], cmap='Blues')
#    axes[i+3].set_xlabel(f'{var_x}')
#    axes[i+3].set_ylabel(f'{var_y}')
#    axes[i+3].set_title(f'Non-Skin - {var_x} vs {var_y}')
